    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                Page.DataBind()
                Session.Remove("IDOCS.application.LoaddataProductStore")
                hddParameterMenuID.Value = HelperLog.LoadMenuID("PRD_ProductStore.aspx")
                Me.ClearSessionPageLoad("PRD_ProductStore.aspx")
...
#Region "SetParameter"
    Public Sub LoadRedirec(ByVal FlagSave As String)
        ...
        Call Redirect("PRD_ProductStore.aspx")
    End Sub
    Public Sub SetParameter(ByVal FlagSave As String)
        Session.Remove("PRD_ProductStore_FS")
        Session.Add("PRD_ProductStore_FS", IIf(FlagSave <> String.Empty, FlagSave, ""))
        Session.Add("PRD_ProductStore_PageInfo", IIf(hddpPageInfo.Value <> String.Empty, hddpPageInfo.Value, ""))
        Session.Add("PRD_ProductStore_Search", IIf(hddpSearch.Value <> String.Empty, hddpSearch.Value, ""))
        Session.Add("PRD_ProductStore_pageLength", IIf(hddpPagingDefault.Value <> String.Empty, hddpPagingDefault.Value, ""))
    End Sub
    Public Sub GetParameter()
        If Session("PRD_ProductStore_FS") IsNot Nothing Then
            If Session("PRD_ProductStore_FS").ToString <> String.Empty Then
                Dim strFS As String = Session("PRD_ProductStore_FS").ToString
                If strFS = "1" Then
            ...
            End If
        End If
        If Session("PRD_ProductStore_PageInfo") IsNot Nothing Then
            If Session("PRD_ProductStore_PageInfo").ToString <> String.Empty Then
                Dim strInfo As String = Session("PRD_ProductStore_PageInfo").ToString
                If strInfo <> String.Empty Then
                    hddpPageInfo.Value = strInfo
                End If
            End If
        End If
        If Session("PRD_ProductStore_Search") IsNot Nothing Then
            If Session("PRD_ProductStore_Search").ToString <> String.Empty Then
                Dim strSearch As String = Session("PRD_ProductStore_Search").ToString
                If strSearch <> String.Empty Then
                    hddpSearch.Value = strSearch
                End If
            End If
        End If
        If Session("PRD_ProductStore_pageLength") IsNot Nothing Then
            If Session("PRD_ProductStore_pageLength").ToString <> String.Empty Then
                Dim strPagingDefault As String = Session("PRD_ProductStore_pageLength").ToString
...
#Region "LoadData"
    ...
    Public Sub LoadInit()
        ...
        lblaName.Text = Me.GetResource("lblaName", "Text", hddParameterMenuID.Value)
        lblaShotName.Text = Me.GetResource("lblaShotName", "Text", hddParameterMenuID.Value)

        TextHd1.Text = Me.GetResource("No", "Text", "1")
        TextHd2.Text = Me.GetResource("Code", "Text", hddParameterMenuID.Value)
        TextHd3.Text = Me.GetResource("Name", "Text", hddParameterMenuID.Value)
        TextHd4.Text = Me.GetResource("ShotName", "Text", hddParameterMenuID.Value)
        ...
        TextFt3.Text = Me.GetResource("Name", "Text", hddParameterMenuID.Value)
        TextFt4.Text = Me.GetResource("ShotName", "Text", hddParameterMenuID.Value)
        ...
        lblBodydelete.Text = Me.GetResource("msg_body_delete", "MSG", "1") & " " & Me.GetResource("lblaCode", "Text", hddParameterMenuID.Value)
    ...
    Public Sub Loaddata()
        Try
            ...
            Dim bl As cSD02GODN = New cSD02GODN
            fillter.Keyword = ""
            ...
            Dim lst As List(Of SD02GODN) = bl.Loaddata(fillter,
                                                       Me.TotalRow,
                                                       Me.CurrentUser.UserID)
            If lst IsNot Nothing Then
                Call SetDataProductStore(lst)
            Else
                Call SetDataProductStore(Nothing)
            End If
            Session.Remove("PRD_ProductStore_FS")
        Catch ex As Exception
...
#Region "Event"
    ...
    Protected Sub btnEdit_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEdit.Click
        Try
            ...
            Dim bl As cSD02GODN = New cSD02GODN
            Dim lc As SD02GODN = bl.GetByID(code,
                                            Me.CurrentUser.UserID)
            If lc IsNot Nothing Then
                hddKeyID.Value = lc.FGDCODE
                txtaCode.Text = lc.FGDCODE
                txtaName.Text = lc.FDESC
                txtaShotName.Text = lc.FSHDESC
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Try
            Dim code As String = Convert.ToString(hddKeyID.Value)
            Dim bl As cSD02GODN = New cSD02GODN
            Dim data As SD02GODN = bl.GetByID(code,
                                                   Me.CurrentUser.UserID)
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSave.Click
        Try
            ...
            Else
                ...
                'Dim lc As SD02GODN = bl.GetByID(txtaCode.Text,
                '                                     Me.CurrentUser.UserID)
...
#Region "Session"
    Public Function GetDataProductStore() As List(Of SD02GODN)
        Try
            Return Session("IDOCS.application.LoaddataProductStore")
        Catch ex As Exception
            Return Nothing
        End Try
    End Function
    Public Function SetDataProductStore(ByVal lc As List(Of SD02GODN)) As Boolean
        Try
            Session.Add("IDOCS.application.LoaddataProductStore", lc)
            Return True
        Catch ex As Exception
            Return False
        End Try
