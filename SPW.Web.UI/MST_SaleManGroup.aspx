<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/MasterPage.Master" CodeBehind="MST_SaleManGroup.aspx.vb" Inherits="SPW.Web.UI.MST_SaleManGroup" EnableEventValidation="false" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                                            ...
                                            <div class="panel panel-primary">
                                                ...
                                                <div class="panel-body">
                                                    <div class="row" style="overflow-x:auto;overflow-y:hidden;">
                                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                                            <%
                                                                Dim lcSalesManGroup As New List(Of LD08SMCT)
                                                                If hddReloadGrid.Value <> String.Empty Then
                                                                    If GetDataSaleManGroup() IsNot Nothing Then
                                                                        lcSalesManGroup = GetDataSaleManGroup()
                                                            %>
                                                            <table id="grdView" class="table table-striped table-bordered table-hover" style="width: 100%">
                                                                ...
                                                                <tbody>
                                                                    <%
                        Dim i As Integer = 0
                        For Each sublcSalesManGroup As LD08SMCT In lcSalesManGroup
                            Dim strb As New StringBuilder()
                                                                    %>
                                                                    <tr>
                                                                        <%
                                                                            strb.Append("<td style='width:1%;text-align: center;'><a id='btnEditTemp' class='btn btn-info glyph-icon icon-edit' href='javascript:CallEditData(&#39;" + IIf(sublcSalesManGroup.FSMTYPE <> String.Empty, sublcSalesManGroup.FSMTYPE, String.Empty) + "&#39;);'></a></td>")
                                                                            Dim arr As String = hddCheckUsedMaster.Value
                                                                            If arr.IndexOf(sublcSalesManGroup.FSMTYPE) = -1 Then
                                                                                strb.Append("<td style='width:1%;text-align: center;'><a id='btnDeleteTemp' class='btn btn-danger glyph-icon icon-trash' href='javascript:ConfirmDelete(&#39;" + IIf(sublcSalesManGroup.FSMTYPE <> String.Empty, sublcSalesManGroup.FSMTYPE, String.Empty) + "&#39;);'></a></td>")
                                                                            Else
                                                                                strb.Append("<td style='width:1%;text-align: center;'></td>")
                                                                            End If
                                                                            strb.Append("<td style='width:1%;text-align: center;'></td>")
                                                                            strb.Append("<td style='width:20%;'>" + IIf(sublcSalesManGroup.FSMTYPE <> String.Empty, sublcSalesManGroup.FSMTYPE, String.Empty) + "</td>")
                                                                            strb.Append("<td style='width:77%;'>" + IIf(sublcSalesManGroup.FSMTYPEDS <> String.Empty, sublcSalesManGroup.FSMTYPEDS, String.Empty) + "</td>")
                                                                ...
                                                                <div class="panel-body <%=IIf((hddKeyID.Value = String.Empty And Me.GetPermission.isAdd) Or (hddKeyID.Value <> String.Empty And Me.GetPermission.isEdit), "", "disabled") %>">
                                                                    ...
                                                                    <div class="row">
                                                                        <div class="col-lg-2 col-md-12 col-sm-12">
                                                                            <asp:Label ID="lblaSalesManGroupCode" runat="server"></asp:Label><asp:Label ID="Label2" runat="server" Text="*" ForeColor="red"></asp:Label>
                                                                        </div>
                                                                        <div class="col-lg-4 col-md-12 col-sm-12" style="text-align: left;">
                                                                            <asp:TextBox ID="txtaSalesManGroupCode" autocomplete="off" runat="server" CssClass="form-control tooltip-button" MaxLength="1" data-toggle="tooltip" data-placement="right" data-original-title='<%# grtt("resTooltipSalesManGroupCode") %>'></asp:TextBox>
                                                                    ...
                                                                    <div class="row">
                                                                        <div class="col-lg-2 col-md-12 col-sm-12">
                                                                            <asp:Label ID="lblaSalesManGroupName" runat="server"></asp:Label><asp:Label ID="Label3" runat="server" Text="*" ForeColor="red"></asp:Label>
                                                                        </div>
                                                                        <div class="col-lg-4 col-md-12 col-sm-12" style="text-align: left;">
                                                                            <asp:TextBox ID="txtaSalesManGroupName" autocomplete="off" runat="server" CssClass="form-control tooltip-button" MaxLength="30" data-toggle="tooltip" data-placement="right" data-original-title='<%# grtt("resTooltipSalesManGroupName") %>'></asp:TextBox>
    ...
    <script type="text/javascript">
        ...
        function CheckData() {
            if ($('#<%=txtaSalesManGroupCode.ClientID%>').val().replace(" ", "") == "") {
                $('#<%=txtaSalesManGroupCode.ClientID%>').addClass("parsley-error")
                document.getElementById('ContentPlaceHolder1_lblMassage1').style.display = "";
                scrollAndFocus('#<%=txtaSalesManGroupCode.ClientID%>');
                return;
            } else {
                $('#<%=txtaSalesManGroupCode.ClientID%>').removeClass("parsley-error")
                document.getElementById('ContentPlaceHolder1_lblMassage1').style.display = "none";
            }
            if ($('#<%=txtaSalesManGroupName.ClientID%>').val().replace(" ", "") == "") {
                $('#<%=txtaSalesManGroupName.ClientID%>').addClass("parsley-error")
                document.getElementById('ContentPlaceHolder1_lblMassage2').style.display = "";
                scrollAndFocus('#<%=txtaSalesManGroupName.ClientID%>');
                return;
            } else {
                $('#<%=txtaSalesManGroupName.ClientID%>').removeClass("parsley-error")
