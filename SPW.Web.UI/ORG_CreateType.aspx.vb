    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                Page.DataBind()
                Session.Remove("IDOCS.application.LoaddataZTypeHouseCon")
                hddParameterMenuID.Value = HelperLog.LoadMenuID("ORG_CreateType.aspx")
                Me.ClearSessionPageLoad("ORG_CreateType.aspx")
...
#Region "SetParameter"
    Public Sub LoadRedirec(ByVal FlagSave As String)
        ...
        Call Redirect("ORG_CreateType.aspx")
    End Sub
    Public Sub SetParameter(ByVal FlagSave As String)
        Session.Remove("ORG_CreateType_FS")
        Session.Add("ORG_CreateType_FS", IIf(FlagSave <> String.Empty, FlagSave, ""))
        Session.Add("ORG_CreateType_PageInfo", IIf(hddpPageInfo.Value <> String.Empty, hddpPageInfo.Value, ""))
        Session.Add("ORG_CreateType_Search", IIf(hddpSearch.Value <> String.Empty, hddpSearch.Value, ""))
        Session.Add("ORG_CreateType_pageLength", IIf(hddpPagingDefault.Value <> String.Empty, hddpPagingDefault.Value, ""))
    End Sub
    Public Sub GetParameter()
        If Session("ORG_CreateType_FS") IsNot Nothing Then
            If Session("ORG_CreateType_FS").ToString <> String.Empty Then
                Dim strFS As String = Session("ORG_CreateType_FS").ToString
                If strFS = "1" Then
            ...
            End If
        End If
        If Session("ORG_CreateType_PageInfo") IsNot Nothing Then
            If Session("ORG_CreateType_PageInfo").ToString <> String.Empty Then
                Dim strInfo As String = Session("ORG_CreateType_PageInfo").ToString
                If strInfo <> String.Empty Then
                    hddpPageInfo.Value = strInfo
                End If
            End If
        End If
        If Session("ORG_CreateType_Search") IsNot Nothing Then
            If Session("ORG_CreateType_Search").ToString <> String.Empty Then
                Dim strSearch As String = Session("ORG_CreateType_Search").ToString
                If strSearch <> String.Empty Then
                    hddpSearch.Value = strSearch
                End If
            End If
        End If
        If Session("ORG_CreateType_pageLength") IsNot Nothing Then
            If Session("ORG_CreateType_pageLength").ToString <> String.Empty Then
                Dim strPagingDefault As String = Session("ORG_CreateType_pageLength").ToString
...
#Region "LoadData"
    ...
    Public Sub Loaddata()
        Try
            ...
            Dim bl As cZTypeHouseCons = New cZTypeHouseCons
            fillter.Keyword = ""
            ...
            Dim lst As List(Of ZTypeHouseCon) = bl.Loaddata(fillter,
                                                            Me.TotalRow,
                                                            Me.CurrentUser.UserID)
            If lst IsNot Nothing Then
                Call SetDataZTypeHouseCon(lst)
            Else
                Call SetDataZTypeHouseCon(Nothing)
            End If
            Session.Remove("ORG_CreateType_FS")
        Catch ex As Exception
...
#Region "Event"
    ...
    Protected Sub btnEdit_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEdit.Click
        Try
            ...
            Dim bl As cZTypeHouseCons = New cZTypeHouseCons
            Dim lc As ZTypeHouseCon = bl.GetByID(code,
                                                 Me.CurrentUser.UserID)
            If lc IsNot Nothing Then
                hddKeyID.Value = lc.TypeHouseConsCode
                txtaCode.Text = lc.TypeHouseConsCode
                txtaName.Text = lc.TypeHouseConsDesc
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Try
            Dim code As String = Convert.ToString(hddKeyID.Value)
            Dim bl As cZTypeHouseCons = New cZTypeHouseCons
            Dim data As ZTypeHouseCon = bl.GetByID(code,
                                                   Me.CurrentUser.UserID)
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSave.Click
        Try
            ...
            Dim bl As cZTypeHouseCons = New cZTypeHouseCons
            Dim data As ZTypeHouseCon = Nothing
            ...
            Else
                ...
                Dim lc As ZTypeHouseCon = bl.GetByID(txtaCode.Text,
                                                     Me.CurrentUser.UserID)
...
#Region "Session"
    Public Function GetDataZTypeHouseCon() As List(Of ZTypeHouseCon)
        Try
            Return Session("IDOCS.application.LoaddataZTypeHouseCon")
        Catch ex As Exception
            Return Nothing
        End Try
    End Function
    Public Function SetDataZTypeHouseCon(ByVal lc As List(Of ZTypeHouseCon)) As Boolean
        Try
            Session.Add("IDOCS.application.LoaddataZTypeHouseCon", lc)
            Return True
        Catch ex As Exception
            Return False
        End Try
