...
Public Class TRN_TransferTargetBytSales
    Inherits BasePage

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                hddParameterMenuID.Value = HelperLog.LoadMenuID("TRN_TransferTargetBytSales.aspx")
...
#Region "Dropdownlist"
    Public Sub LoadDropdownlist()
        ...
        Call LoadSalesman(ddlsSaleMan, ddlsProject.SelectedValue, "S")
    End Sub
    ...
    Public Sub LoadSalesman(ByVal ddl As DropDownList,
                            ByVal strProject As String,
                           ByVal strType As String)
        Dim bl As cTransferTargetBytSales = New cTransferTargetBytSales
        ddl.Items.Clear()
        Try
            If strType = "A" Then
                ddl.Items.Insert(0, New ListItem(GetResource("msg_select_all", "MSG"), ""))
            ...
            If lc IsNot Nothing Then
                If lc.Count > 0 Then
                    ddl.DataValueField = "FSMCODE"
                    If Me.CurrentPage.ToString.ToUpper = "EN" Then
                        ddl.DataTextField = "FSMNAME"
                    Else
                        ddl.DataTextField = "FSMNAME"
                    End If
                    ddl.DataSource = lc
    ...
    Protected Sub ddlsProject_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlsProject.SelectedIndexChanged
        Try
            If ddlsProject.SelectedIndex = 0 Then
                hddsProject.Value = ""
            Else
                hddsProject.Value = ddlsProject.SelectedValue
            End If
            Call LoadSalesman(ddlsSaleMan, ddlsProject.SelectedValue, "S")
            Call ClearText()
        ...
        End Try
    End Sub
    Protected Sub ddlsSaleMan_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlsSaleMan.SelectedIndexChanged
        Try
            Dim strProject As String = String.Empty
            Dim strSalesMan As String = String.Empty
            'Call SetCiterail(txtsProject.Text, strProject)
            strProject = ddlsProject.SelectedValue
            'Call SetCiterail(txtsSaleMan.Text, strSalesMan)
            strSalesMan = ddlsSaleMan.SelectedValue
            If strProject <> String.Empty And
               strSalesMan <> String.Empty Then
                Call Loaddata()
            Else
                Call ClearText()
            End If
        Catch ex As Exception
            HelperLog.ErrorLog(CurrentUser.UserID, hddParameterMenuID.Value, Request.UserHostAddress(), "ddlsSaleMan_SelectedIndexChanged", ex)
        End Try
    End Sub
...
#Region "Loaddata"
    Public Sub LoadInit()
        ...
        lblsSalesman.Text = Me.GetResource("lblsSalesman", "Text", hddParameterMenuID.Value)
        'lblsStdPrice.Text = Me.GetResource("lblsStdPrice", "Text", hddParameterMenuID.Value)
    ...
    Public Sub Loaddata()
        Try
            ...
            Dim bl As cTransferTargetBytSales = New cTransferTargetBytSales
            Dim lcSalesMan As List(Of LD422PTR) = bl.Loaddata(strProject,
                                                              strSalesMan,
                                                              txtsYear.Text)
            If lcSalesMan IsNot Nothing Then
                If lcSalesMan.Count > 0 Then
                    For Each sublcSalesMan As LD422PTR In lcSalesMan
