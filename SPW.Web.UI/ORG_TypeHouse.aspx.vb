    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not IsPostBack Then
                Page.DataBind()
                Session.Remove("IDOCS.application.LoaddataTypeHouse")
                hddParameterMenuID.Value = HelperLog.LoadMenuID("ORG_TypeHouse.aspx")
                Me.ClearSessionPageLoad("ORG_TypeHouse.aspx")
                Call LoadInit()
                Call DefaultAllData()
...
#Region "SetParameter"
    Public Sub LoadRedirec(ByVal FlagSave As String)
        ...
        Call Redirect("ORG_TypeHouse.aspx")
    End Sub
    Public Sub SetParameter(ByVal FlagSave As String)
        Session.Remove("ORG_TypeHouse_FS")
        Session.Add("ORG_TypeHouse_FS", IIf(FlagSave <> String.Empty, FlagSave, ""))
        Session.Add("ORG_TypeHouse_PageInfo", IIf(hddpPageInfo.Value <> String.Empty, hddpPageInfo.Value, ""))
        Session.Add("ORG_TypeHouse_Search", IIf(hddpSearch.Value <> String.Empty, hddpSearch.Value, ""))
        Session.Add("ORG_TypeHouse_pageLength", IIf(hddpPagingDefault.Value <> String.Empty, hddpPagingDefault.Value, ""))
    End Sub
    Public Sub GetParameter()
        If Session("ORG_TypeHouse_FS") IsNot Nothing Then
            If Session("ORG_TypeHouse_FS").ToString <> String.Empty Then
                Dim strFS As String = Session("ORG_TypeHouse_FS").ToString
                If strFS = "1" Then
            ...
            End If
        End If
        If Session("ORG_TypeHouse_PageInfo") IsNot Nothing Then
            If Session("ORG_TypeHouse_PageInfo").ToString <> String.Empty Then
                Dim strInfo As String = Session("ORG_TypeHouse_PageInfo").ToString
                If strInfo <> String.Empty Then
                    hddpPageInfo.Value = strInfo
                End If
            End If
        End If
        If Session("ORG_TypeHouse_Search") IsNot Nothing Then
            If Session("ORG_TypeHouse_Search").ToString <> String.Empty Then
                Dim strSearch As String = Session("ORG_TypeHouse_Search").ToString
                If strSearch <> String.Empty Then
                    hddpSearch.Value = strSearch
                End If
            End If
        End If
        If Session("ORG_TypeHouse_pageLength") IsNot Nothing Then
            If Session("ORG_TypeHouse_pageLength").ToString <> String.Empty Then
                Dim strPagingDefault As String = Session("ORG_TypeHouse_pageLength").ToString
...
#Region "LoadData"
    ...
    Public Sub LoadInit()
        ...
        lblaTypeCode.Text = Me.GetResource("lblaTypeCode", "Text", hddParameterMenuID.Value)
        lblaTypeName.Text = Me.GetResource("lblaTypeName", "Text", hddParameterMenuID.Value)

        TextHd1.Text = Me.GetResource("No", "Text", "1")
        TextHd2.Text = Me.GetResource("TypeCode", "Text", hddParameterMenuID.Value)
        TextHd3.Text = Me.GetResource("TypeName", "Text", hddParameterMenuID.Value)
        ...
        TextFt2.Text = Me.GetResource("TypeCode", "Text", hddParameterMenuID.Value)
        TextFt3.Text = Me.GetResource("TypeName", "Text", hddParameterMenuID.Value)
        ...
        lblBodydelete.Text = Me.GetResource("msg_body_delete", "MSG", "1") & " " & Me.GetResource("lblaTypeCode", "Text", hddParameterMenuID.Value)
    ...
    Public Sub Loaddata()
        Try
            ...
            Dim bl As cTypeHouse = New cTypeHouse
            fillter.Keyword = ""
            ...
            Dim lst As List(Of vw_TypeHouse_Join) = bl.Loaddata(fillter,
                                                                Me.TotalRow,
                                                                Me.CurrentUser.UserID)
            If lst IsNot Nothing Then
                Call SetDataTypeHouse(lst)
            Else
                Call SetDataTypeHouse(Nothing)
            End If
            Session.Remove("ORG_TypeHouse_FS")
        Catch ex As Exception
...
#Region "Event"
    ...
    Protected Sub btnEdit_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEdit.Click
        Try
            ...
            Dim bl As cTypeHouse = New cTypeHouse
            Dim lc As SD03TYPE = bl.GetTypeByID(code,
                                                  Me.CurrentUser.UserID)
            If lc IsNot Nothing Then
                hddKeyID.Value = lc.FTYCODE
                txtaTypeCode.Text = lc.FTYCODE
                txtaTypeName.Text = lc.FDESC

                txtaTypeCode.Enabled = False
            End If
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        Try
            Dim code As String = Convert.ToString(hddKeyID.Value)
            Dim bl As cTypeHouse = New cTypeHouse
            Dim data As SD03TYPE = bl.GetTypeByID(code,
                                                    Me.CurrentUser.UserID)
            If data IsNot Nothing Then
                If Not bl.TypeDelete(code,
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSave.Click
        Try
            ...
            Dim bl As cTypeHouse = New cTypeHouse
            Dim data As SD03TYPE = Nothing
            If hddKeyID.Value <> "" Then
                If Not bl.TypesEdit(hddKeyID.Value,
                                    txtaTypeName.Text,
                                    Me.CurrentUser.UserID) Then
                    Call OpenDialog()
            ...
            Else
                ...
                Dim lc As SD03TYPE = bl.GetTypeByID(txtaTypeCode.Text,
                                                    Me.CurrentUser.UserID)
                If lc Is Nothing Then
                    If Not bl.TypeAdd(txtaTypeCode.Text,
                                      txtaTypeName.Text,
                                      Me.CurrentUser.UserID) Then
...
#Region "Session"
    Public Function GetDataTypeHouse() As List(Of vw_TypeHouse_Join)
        Try
            Return Session("IDOCS.application.LoaddataTypeHouse")
        Catch ex As Exception
            Return Nothing
        End Try
    End Function
    Public Function SetDataTypeHouse(ByVal lcTypeHouse As List(Of vw_TypeHouse_Join)) As Boolean
        Try
            Session.Add("IDOCS.application.LoaddataTypeHouse", lcTypeHouse)
            Return True
        Catch ex As Exception
            Return False
        End Try
