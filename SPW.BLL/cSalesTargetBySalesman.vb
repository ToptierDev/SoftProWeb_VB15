...
Public Class cSalesTargetBySalesman

#Region "TRN_SaleTargetBySalesman.aspx"

    Public Function Loaddata(ByVal pProject As String,
                             ByVal pSalesMan As String,
                             ByVal pYear As String) As List(Of LD422TSC)

        Using db As New PNSWEBEntities
            Dim lcSalesTarget As List(Of LD422TSC) = db.LD422TSC.Where(Function(s) s.FCUCODE = pProject _
                                                                               And s.FYEAR = pYear _
                                                                               And s.FSMCODE = pSalesMan).OrderBy(Function(s) s.FCUCODE).ToList
    ...
    Public Function addData(ByVal dt As DataTable,
        ...
        Using db As New PNSWEBEntities
            Dim lc As List(Of LD422TSC) = db.LD422TSC.Where(Function(s) s.FCUCODE = pProject _
                                                                    And s.FYEAR = pYear _
                                                                    And s.FSMCODE = pSalesMan).ToList
            For Each e As LD422TSC In lc
                db.LD422TSC.Remove(e)
            Next
            If dt IsNot Nothing Then
                Dim m As New LD422TSC
                m.FYEAR = CInt(pYear)
                m.FCUCODE = pProject
                m.FSMCODE = pSalesMan
                m.FTYCODE = ""
...
#End Region
    ...
    Public Function LoadSalesman(ByVal pKeyID As String,
                                 ByVal pProject As String) As List(Of String)
        Using db As New PNSWEBEntities
            If pProject = String.Empty Then
                If pKeyID = String.Empty Then
                    Dim qury = (From m In db.LD01SMAN.OrderBy(Function(s) s.FSMCODE)
                                Select m.FSMCODE & "-" & m.FSMNAME).ToList()

                    Return qury.ToList()
                ElseIf pKeyID <> String.Empty Then
                    Dim qury = (From m In db.LD01SMAN.OrderBy(Function(s) s.FSMCODE)
                                Select m.FSMCODE & "-" & m.FSMNAME) _
                               .Where(Function(s) s.ToUpper.Replace(" ", "").Contains(pKeyID.ToUpper.Replace(" ", ""))).ToList()
            ...
            Else
                If pKeyID = String.Empty Then
                    Dim qury = (From m In db.LD01SMAN.Where(Function(s) s.FREPRJNO.ToUpper.Replace(" ", "").Contains(pProject.ToUpper.Replace(" ", ""))).OrderBy(Function(s) s.FSMCODE)
                                Select m.FSMCODE & "-" & m.FSMNAME).ToList()
